...
void iniciar_missao_ponte(EstadoJogo *e) {
    ...
    // Posições visuais das 5 rachaduras na ponte
    ...
    // Posição inicial do Chico na ponte
...
void processar_input_ponte(EstadoJogo *e) {
    ...
    // [1] a [5] — reparar posição da ponte
...
void atualizar_ponte(EstadoJogo *e, float dt) {
    ...
    if (!e->turno_jogador) {
        ...
        // Nova rachadura aleatória a cada turno
        int tentativas = 0;
        while (tentativas < 5) {
            int pos = rand() % 5;
        ...
        // Ataque ao Chico a cada 2 turnos
